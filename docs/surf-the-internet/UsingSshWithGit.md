...
每次都需要加载ssh证书，应该有办法启动bash时自动加载，github的官网文档上好像就有相关内容
...
现在换到了虚拟机+remote的方式，更新下

linux下的ssh配置方便得多，先ssh-keygen弄一对密钥，现在都默认ed25519加密了

然后把id_ed25519.pub上传到你的github account
...
或者去配置一下ssh的默认文件`~/.ssh/config` 我没有试过
